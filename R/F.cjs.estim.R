...
r.package.version <- paste( R.version$major, R.version$minor, sep="")
run.date <- Sys.time()
...
# return the X and Y matricies.  After this, the x and Y matricies are in huge NAN by (ns*nx) matricies.
covars <- F.cr.model.matrix( capture, survival )  
...
if( missing(cap.init) ){
	cap.init <- rep(0,nx+covars$cap.intercept)
} else if(length(cap.init) < (nx+covars$cap.intercept) ){
	cap.init <- c(cap.init, rep(0, nx+covars$cap.intercept-length(cap.init)))
} 

if( missing(sur.init) ){
	sur.init <- rep(0,ny+covars$sur.intercept)
} else if(length(sur.init) < (ny+covars$sur.intercept) ){
	sur.init <- c(sur.init, rep(0, ny+covars$sur.intercept-length(sur.init)))
} 

# 	Fix up the covariate matricies that have intercepts.  After call to F.cr.model.matrix, intercepts are 
# 	a single column.  Should be ns columns. 

if( covars$cap.intercept ){
	covars$capX <- cbind( matrix( 1, nan, ns ), covars$capX[,-1] )
	nx <- nx + 1
} 
if( covars$sur.intercept ){
	covars$survX <- cbind( matrix( 1, nan, ns ), covars$survX[,-1] )
	ny <- ny + 1
} 
...
parameters <- se.param <- se1.param <- rep(0, nx + ny )
...
ans <- .Fortran( "mrawin", as.integer(nan), as.integer(ns), as.integer(nx), 
		as.integer(ny), as.integer(ng), as.integer(histories), as.integer(group), 
		as.integer(algorithm), as.integer(cov.meth), as.integer(nhat.v.meth), 
		as.double(covars$capX), as.double(covars$survX), 
		as.double(cap.init), as.double(sur.init), as.double(loglik), as.double(deviance), 
		as.double(aic), as.double(qaic), as.double(vif), as.double(chisq.vif), 
		as.double(df.vif), as.double(parameters),
		as.double(se.param), as.double(se1.param), as.double(covariance), as.double(p.hat), 
		as.double(se.p.hat), as.double(s.hat), as.double(se.s.hat), as.double(n.hat), 
		as.double(se.n.hat), as.integer(exit.code), as.integer(cov.code), 
		as.integer(df.estimated), as.double(intervals), PACKAGE="mra") 
...
if( covars$cap.intercept ){
	names(capcoef) <- c("(Intercept)", covars$cap.vars )
	names(se.capcoef) <- c("(Intercept)", covars$cap.vars )
} else {
	# No intercept
	names(capcoef) <- c(covars$cap.vars)
	names(se.capcoef) <- c(covars$cap.vars)
}

if( covars$sur.intercept ){
	names(surcoef) <- c("(Intercept)", covars$sur.vars )
	names(se.surcoef) <- c("(Intercept)", covars$sur.vars )
} else {
	# No intercept
	names(surcoef) <- c(covars$sur.vars)
	names(se.surcoef) <- c(covars$sur.vars)
}	
...
aux <- list( call=cr.call, nan=nan, ns=ns, nx=length(capcoef), ny=length(surcoef), 
		cov.name=c(names(capcoef), names(surcoef)), 
		ic.name=hist.name, mra.dll.version="5.3",  
...
ans <- list( histories=histories, aux=aux, intervals=intervals[1:(ns-1)], 
	loglike=loglik, deviance=deviance, aic=aic, qaic=qaic, aicc=aicc, qaicc=qaicc,
	ebc=ebc, vif=vif, chisq.vif=chisq.vif, vif.df=df.vif, 
	parameters=parameters, se.param=se.param, se1.param=se1.param, 
	capcoef=capcoef, se.capcoef=se.capcoef, surcoef=surcoef, 
	se.surcoef=se.surcoef, covariance=covariance,
	p.hat=p.hat, se.p.hat=se.p.hat, s.hat=s.hat, se.s.hat=se.s.hat, 
	df=df, message=message, exit.code=exit.code, cov.code=cov.code, cov.meth=cov.meth, 
	n.hat = n.hat, se.n.hat=se.n.hat, n.hat.lower = lower.ci, n.hat.upper = ceiling(upper.ci),
	n.hat.conf = 0.95, nhat.v.meth = nhat.v.meth, num.caught=num.observed)
class(ans) <- c("cjs", "cr")

...
!
	...
	!INTEGER, PARAMETER :: I1B = SELECTED_INT_KIND(2)
	!INTEGER, PARAMETER :: SP = KIND(1.0)
	!INTEGER, PARAMETER :: DP = KIND(1.0D0)     ! DP needed for numerical recipies routines
...
!	ptr_survX = pointer to survival covariate 3-D array, nan X ns X ny
	...
	real(kind=dbl), dimension(:,:,:), pointer :: ptr_capX, ptr_survX
...
! ----------------------------------------------------------------------------
subroutine mrawin( nan, ns, nx, ny, ng, hist, group, algorithm, cov_meth, nhat_v_meth, & 
		capX, survX, &
		cap_init, sur_init, loglik, deviance, aic, qaic, c_hat, &
		chisq_vif, df_vif, parameters, &
		se_param, se1_param, covariance, p_hat, se_p_hat, s_hat, se_s_hat, &
		n_hat, se_n_hat, &
		exit_code, pos_def_code, df, intervals )
!
!	Main routine to do Mark-Recapture Analysis.  This is called from
!	S with input parameters.
!
!	Note that the previous version, MRAWIN4, used direct assess files on 
...
!	se_param and se1_param = vector of standard errors
...
!	Output parameters
	real(kind=dbl), intent(inout) :: loglik, deviance, aic, qaic, c_hat, &
	                                 chisq_vif, df_vif
	real(kind=dbl), intent(inout), dimension(nx+ny) :: parameters, se_param, se1_param
...
!	---- Open a log file, primarily for debugging
	OPEN(6,FILE="mrawin5.log",status="replace",iostat=ioerr)
	if ( ioerr /= 0 ) then
...
!	---- Do the estimation using supplied initial values
	...
	call estim(np, algorithm, cov_meth, parameters, loglik, covariance, exit_code, &
		pos_def_code, df)  
...
!		---- Calculate probability of capture and survival, and SE for both estimates
		call probs_and_vars(nan,ns,np,parameters,covariance,p_hat,s_hat, se_p_hat,se_s_hat)
...
end subroutine mrawin



! ---------------------------------------------------------------------------------------------

subroutine estim(np, algorithm, cov_meth, parameters, loglik, covariance, exit_code, cov_npd, df )
!
...
!	character(len=*), intent(inout) :: mess
	...
	real(kind=dbl), dimension(np) :: eps ! required precision in each element of parameters
	...
	external CJS  ! objective function for minimization
	...
	select case (algorithm) 
		...
		case (:1, 3:)
			...
			call VA09AD(CJS,np,parameters,loglik,g,h,W, -2.0_dbl, EPS, 1, 1000, 10, exit_code)
...
SUBROUTINE CJS(p,beta,lnlik,grad)
!
...
!real(kind=dbl) function CJS_loglik(p, beta)  THIS DOES NOT WORK IN GFORTRAN
!
...
!	ptr_capX = pointer to capture covariate 3-D array, nan X ns X nx
!	ptr_capY = pointer to survival covariate 3-D array, nan X ns X ny
!
...
!	mrawin.
	...
	integer :: i, j, init1, init2, k, ii, jj
...
!
	...
	do i=1,p
		deltai=(delta/2.0_dbl)*(1.0_dbl + abs(beta2(i)))
...
subroutine probs_and_vars(nan,ns,np,parameters,covariance,p_hat,s_hat, se_p_hat,se_s_hat)
!
...
!	---- Open a log file to store intermediate tables that may be of use
	OPEN(6,FILE="mrawin5_gof.log",status="replace",iostat=ioerr)
	if ( ioerr /= 0 ) then
		! Cannot open log file
		stop
	end if
...
!	ierr = error code returned from inverse procedure
	...
	integer :: i, j
	real(kind=dbl), dimension(np) :: beta2
